...
interface AssignOrderUseCase {
    execute(orderAssignDto: OrderAssignDto): Promise<Order>;
}

export class AssignOrder implements AssignOrderUseCase {
    constructor(private readonly orderRepository: OrderRepository) {}

    async execute(orderAssignDto: OrderAssignDto): Promise<Order> {
        const order = await this.orderRepository.update(orderAssignDto);
        return {
            id: order.id,
